  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      ...
      <p className="mb-5 text-sm text-gray-500">
        {reports.length} bejelentés
        {filterSeverity || filterStatus ? " (szűrve)" : ""}
      </p>

      {/* Szűrők */}
      <div className="mb-6 flex flex-wrap gap-2">
        <div className="flex flex-wrap gap-1.5">
          <span className="mr-1 self-center text-xs font-semibold text-gray-400">
            Súlyosság:
          </span>
      ...
      {reports.length === 0 && (
        <p className="mt-8 text-center text-gray-400">
          Nincs bejelentés a szűrési feltételeknek megfelelően.
        </p>
      )}
      ...
      <div className="space-y-5">
        {reports.map((r) => {
          const categoryLabel =
            USER_REPORT_CATEGORIES.find((c) => c.value === r.reason)?.label ??
            r.reason;
...
async function ReportCard({
  ...
  return (
    ...
    >
      ...
      {/* Metaadatok */}
      <div className="mb-3 grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-gray-500">
        <div>
          <span className="font-semibold">Bejelentő:</span>{" "}
          {r.reporter_user_id.slice(0, 8)}…
        </div>

        <div>
          <span className="font-semibold">Bejelentett:</span>{" "}
          {r.reported_user_id.slice(0, 8)}…
        </div>

        <div className="col-span-2">
          <span className="font-semibold">Kategória:</span> {categoryLabel}
        </div>
        ...
        {r.retention_until && (
          <div className="col-span-2">
            ...
            {r.legal_hold && (
              <span className="ml-2 rounded bg-red-100 px-1.5 py-0.5 font-bold text-red-700">
                LEGAL HOLD
              </span>
            )}
          </div>
            ...
            {auditLog.map((entry) => (
              <div key={entry.id} className="text-xs text-gray-500">
                ...
                {entry.previous_severity && entry.new_severity && (
                  <span className="ml-1 text-gray-400">
                    [súlyosság: {entry.previous_severity} →{" "}
                    {entry.new_severity}]
                  </span>
                )}
