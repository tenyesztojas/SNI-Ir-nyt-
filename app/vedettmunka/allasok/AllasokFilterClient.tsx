...
// Szövegalapú szűrő chip-ek (piktogram nélkül)
const VM_FILTERS: { label: string; param: string }[] = [
  { label: "Fokozatos betanítás",              param: "betanitas"  },
  { label: "Kijelölt segítő",                  param: "mentor"     },
  { label: "Csendes környezet",                param: "quiet"      },
  { label: "Kis csapat",                       param: "kis_csapat" },
  { label: "Kevés beszélgetés",                param: "low_verbal" },
  { label: "Írásban is megkapod a feladatot",  param: "written"    },
  { label: "Részmunkaidő",                     param: "part_time"  },
  { label: "Rugalmas munkaidő",                param: "flex"       },
  { label: "Otthoni munkavégzés",              param: "home_office"},
  { label: "Kiszámítható munkarend",           param: "pred_sched" },
];
  ...
  return (
    <form method="GET" className="flex flex-col gap-5 rounded-2xl border border-gray-100 bg-white p-5 shadow-soft">
      ...
      <label className="flex flex-col gap-1">
        ...
        >
          <option value="">Mindegy</option>
          <option value="munkahelyen">Helyszínen</option>
          <option value="otthonrol">Otthonról</option>
          <option value="hibrid">Helyszín és otthon</option>
        </select>
      </label>

      {/* Szövegalapú szűrő chip-ek */}
      <div>
        <span className="text-xs font-semibold text-gray-500 block mb-2">
          Milyen lehetőséget keresel?
        </span>
        <div className="flex flex-col gap-1.5">
          {VM_FILTERS.map(({ label, param }) => {
